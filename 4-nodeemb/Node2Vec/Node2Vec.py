...
class Node2Vec:
    """Node2Vec algorithm implemented using PyTorch."""

    def __init__(self, graph: dict, walk_length: int, num_walks: int, embedding_dim: int, window_size: int, lr: float, num_epochs: int, p: float, q: float):
        """
    ...
    def node2vec_walk(self, start_node: str) -> list:
        """Perform a Node2Vec biased random walk starting from the given node."""
        walk = [start_node]
        for _ in range(self.walk_length - 1):
            cur = walk[-1]
            cur_neighbors = self.graph[cur]
            if len(cur_neighbors) > 0:
                if len(walk) == 1:
                    walk.append(random.choice(cur_neighbors))
                else:
                    prev = walk[-2]
                    probabilities = self.get_transition_probabilities(prev, cur, cur_neighbors)
                    walk.append(random.choices(cur_neighbors, weights=probabilities, k=1)[0])
            else:
                break
        return walk

    def get_transition_probabilities(self, prev: str, cur: str, neighbors: list) -> list:
        """Compute transition probabilities for Node2Vec biased random walk."""
        probabilities = []
...
with open('/mnt/data/words_dat.txt', 'r') as f:
    lines = f.readlines()
...
# Convert the graph to an adjacency list dictionary format for the Node2Vec class
graph_dict = defaultdict(list)
for node in graph.nodes():
    graph_dict[node] = list(graph.neighbors(node))

# Initialize and train the Node2Vec model with the generated graph
node2vec = Node2Vec(graph_dict, walk_length=10, num_walks=80, embedding_dim=64, window_size=5, lr=0.025, num_epochs=10, p=1, q=1)
node2vec.train()
...
plt.savefig(f'figures/Node2Vec_PCA.png')
plt.close()